...
public class RegistrationController {
    ...
    public ResponseEntity<?> bulkUpdateRegistration(@RequestBody Map<String, Object> request)
    {
        try
        {
            Object idsObject = request.get("registrationIds");

            if (!(idsObject instanceof List<?> idsList)) {
                  return ResponseEntity.badRequest().body("Invalid registrationIds");
            }

           List<Long> registrationIds = idsList.stream()
