...
def df_to_records(df, columns=None):
    """
    Convert DataFrame to JSON-safe list of dicts.
    If columns provided, keep only those columns if they exist.
    """
...
def build_json_response(
    ...
    best_bowlers_df,
    best_allrounders_df,
    best_wicketkeepers_df,
    final_squad_df
):
    """
    Build final chatbot / API JSON response.
    """
    return {
        "success": success,
        "dataset_info": {
            "original_columns": info.get("original_columns", []),
            "mapped_columns": info.get("mapped_columns", {}),
            "created_columns": info.get("created_columns", []),
            "standardization_warnings": info.get("warnings", [])
        },
        "errors": errors,
        "warnings": warnings,

        "best_batsmen": df_to_records(
            best_batsmen_df,
            ["Name", "Role", "selection_probability"]
        ),
        "best_bowlers": df_to_records(
            best_bowlers_df,
            ["Name", "Role", "selection_probability"]
        ),
        "best_allrounders": df_to_records(
            best_allrounders_df,
            ["Name", "Role", "selection_probability"]
        ),
        "best_wicketkeepers": df_to_records(
            best_wicketkeepers_df,
            ["Name", "Role", "selection_probability"]
        ),
        "final_squad": df_to_records(
            final_squad_df,
            ["Name", "Role", "selection_probability"]
        )
    }
...
def generate_squad_from_csv(csv_path, min_last_year=None, verbose=True):
    ...
    Returns JSON-safe dict for chatbot / backend use.
    """
    ...
    if errors:
        ...
        return build_json_response(
            ...
            errors=errors,
            warnings=warnings,
            best_batsmen_df=pd.DataFrame(),
            best_bowlers_df=pd.DataFrame(),
            best_allrounders_df=pd.DataFrame(),
            best_wicketkeepers_df=pd.DataFrame(),
            final_squad_df=pd.DataFrame()
        )
    ...
    # if freshness filter removes everything
    if len(df) == 0:
        warnings = warnings + [f"No players left after applying Last >= {min_last_year} filter."]
        return build_json_response(
            success=False,
            info=info,
            errors=["No players available after filtering."],
            warnings=warnings,
            best_batsmen_df=pd.DataFrame(),
            best_bowlers_df=pd.DataFrame(),
            best_allrounders_df=pd.DataFrame(),
    ...
    return build_json_response(
        ...
        errors=[],
        warnings=warnings,
        best_batsmen_df=best_batsmen,
        best_bowlers_df=best_bowlers,
        best_allrounders_df=best_allrounders,
        best_wicketkeepers_df=best_wicketkeepers,
        final_squad_df=final_squad
    )

# =========================================================
# CLI ENTRY
# Usage:
# python generalized_squad_selector.py <csv_path> [min_last_year]
# =========================================================
if __name__ == "__main__":
    ...
    try:
        result = generate_squad_from_csv(
            csv_path=csv_path,
            min_last_year=min_last_year,
            verbose=False
        )
